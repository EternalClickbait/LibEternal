...
namespace LibEternal.Unity
{
	...
	public abstract class SingletonMonoBehaviour<T> : MonoBehaviour where T : SingletonMonoBehaviour<T>
	{
		...
		public static T Instance
		{
			get
			{
				...
				if (IsDestroyed)
				{
					Log.Warning("Attempt to access destroyed {Type} singleton", typeof(T));
					return null;
				}

				Log.Information("Singleton Instance of {Type} not set", typeof(T));
				...
				//Finding an existing instance
				{
					...
					{
						Log.Warning("Singleton of type {SingletonName} not found. Ensure that it is placed on a valid GameObject", typeof(T).Name);
					...
					if (instances.Length > 1)
					{
						Log.Warning("More than one singleton of type {SingletonType} was found ({InstancesCount}): {AllInstances}", typeof(T), instances.Length, instances);
				...
				throw new InvalidCodePathException(errorMessage);
			}
		}
	...
	#region Unity3d Messages - DO NOT OVERRRIDE / IMPLEMENT THESE METHODS in child classes!
		...
		private void Awake()
		{
			...
			{
				Log.Warning("Duplicate {Type} Singleton created", typeof(T));
