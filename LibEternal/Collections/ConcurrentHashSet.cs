...
namespace LibEternal.Collections
{
	...
	public class ConcurrentHashSet<T> : IReadOnlyCollection<T>, ICollection<T>
	{
		...
		/// </exception>
		public ConcurrentHashSet([NotNull] IEnumerable<T> collection, IEqualityComparer<T> comparer)
			: this(comparer)
		{
		...
		/// </exception>
		public ConcurrentHashSet(int concurrencyLevel, [NotNull] IEnumerable<T> collection, IEqualityComparer<T> comparer)
		...
		private ConcurrentHashSet(int concurrencyLevel, int capacity, bool growLockArray, [CanBeNull] IEqualityComparer<T> comparer)
		{
