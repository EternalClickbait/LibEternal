...
namespace LibEternal.Callbacks.Generic
{
	...
	/// </summary>
	[PublicAPI]
	public class SafeFunc<T1, T2, TReturn>
	{
		/// <inheritdoc cref="SafeFunc{TReturn}.callbacks" />
		private readonly List<Func<T1, T2, TReturn>> callbacks;

		/// <inheritdoc cref="SafeFunc{TReturn}(List{Func{TReturn}})" />
		public SafeFunc([CanBeNull] List<Func<T1, T2, TReturn>> callbacks = null)
		{
			this.callbacks = callbacks ?? new List<Func<T1, T2, TReturn>>();
		}

		/// <inheritdoc cref="SafeFunc{TReturn}.Event" />
		public event Func<T1, T2, TReturn> Event
		{
		...
		public (List<Exception> exceptions, List<TReturn> results) InvokeSafe(T1 param1, T2 param2)
		{
			...
			{
				Func<T1, T2, TReturn> action = callbacks[i];
				try
				{
					results.Add(action.Invoke(param1, param2));
				}
