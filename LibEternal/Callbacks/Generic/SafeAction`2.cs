...
namespace LibEternal.Callbacks.Generic
{
	...
	public sealed class SafeAction<T0, T1>
	{
		/// <summary>
		...
		public readonly ReadonlyHashSet<Action<T0, T1>> Callbacks;
		
		/// <summary>
		///     The constructor to instantiate a new <see cref="SafeAction{T0, T1}" />
		/// </summary>
		/// <param name="callbacks">An optional <see cref="List{T}" /> of <see cref="Action" />s to use as a base</param>
		public SafeAction([CanBeNull] IEnumerable<Action<T0, T1>> callbacks = null)
		{
		    ...
		    Callbacks = new ReadonlyHashSet<Action<T0, T1>>(this.callbacks);
		}
